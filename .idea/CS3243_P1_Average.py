...
class Node(object):
    ...
    def __get_heuristic_cost(self):
        # Implement the heuristics. Try average of Manhatten distance and  euclidean distance.
            return (self.__get_manhatten() + self.__get_euclidean())/2;
    ...
    def __get_euclidean(self):
        ...
        for i in range (0,self.n):
            for j in range (0,self.n):
                ...
                if value != 0:
                    correct_index = [(value - 1)//self.n, (value - 1) % self.n]
                    power_x = (correct_index[0] - i) ** 2
                    power_y = (correct_index[1] - j) ** 2
                    distance += math.sqrt(power_x + power_y)
        return distance
